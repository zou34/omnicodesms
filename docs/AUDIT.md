...
**Top 5 des points restants à traiter en priorité** :
1. Absence totale de rate limiting / anti-bruteforce (inscription, connexion, checkout).
2. Aucun test automatisé (unitaire, intégration, e2e).
3. Panneau admin entièrement sur données mockées — aucun accès réel aux utilisateurs/transactions/commandes.
4. Absence de `robots.txt`, `sitemap`, métadonnées Open Graph, et de `metadata` par page sur les pages à plus fort trafic (landing, login, register).
5. Sidebar admin non responsive (illisible sur mobile).
...
| # | Constat | Fichier | Sévérité |
|---|---|---|---|
| 1 | Aucun `robots.txt`, `sitemap.xml`, ni métadonnées Open Graph/Twitter nulle part ; `metadata` racine limitée à `title`+`description` | `app/layout.tsx`, racine du projet | **Élevé** |
| 2 | `/login` et `/register` sont des composants client (`"use client"`) et ne peuvent donc pas exporter `metadata` en l'état — blocage architectural, pas un simple oubli | `app/login/page.tsx`, `app/register/page.tsx` | **Élevé** |
| 3 | Aucun `loading.tsx`, `error.tsx`, `not-found.tsx` ni `global-error.tsx` dans toute l'arborescence `app/` — aucune page d'erreur/404 personnalisée, aucun garde-fou App Router en cas d'échec serveur | tout `app/` | **Élevé** |
...
1. Ajouter `robots.txt` + `sitemap.ts` + métadonnées Open Graph de base.
...
4. **Mettre en place `loading.tsx`/`error.tsx`/`not-found.tsx`** sur les segments clés (`dashboard`, `admin`, racine).
5. **Démarrer une suite de tests minimale** (Vitest sur `lib/`, Playwright sur les parcours critiques).
